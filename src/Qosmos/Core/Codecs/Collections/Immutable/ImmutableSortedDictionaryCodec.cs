...
/// <summary>
/// A codec for encoding and decoding <see cref="ImmutableSortedDictionary{TKey, TValue}"/> objects to and from JSON.
/// </summary>
/// <typeparam name="TKey">The type of the dictionary keys. Must be non-nullable.</typeparam>
/// <typeparam name="TValue">The type of the dictionary values.</typeparam>
public sealed class ImmutableSortedDictionaryCodec<TKey, TValue> : Codec<ImmutableSortedDictionary<TKey, TValue>>
    where TKey : notnull
{
    ...
    public ImmutableSortedDictionaryCodec(Codec<TValue> codec, Func<TKey, string> keyToStringMethod, Func<string, TKey> stringToKeyMethod)
    {
        _codec = codec;
    ...
    /// <summary>
    /// Decodes a JSON object into an <see cref="ImmutableSortedDictionary{TKey, TValue}"/>.
    /// </summary>
    /// <param name="reader">The <see cref="Utf8JsonReader"/> to read the JSON data from.</param>
    /// <returns>An <see cref="ImmutableSortedDictionary{TKey, TValue}"/> instance, or null if the JSON token is null.</returns>
    /// <exception cref="JsonException">Thrown if the JSON structure is invalid.</exception>
    public override ImmutableSortedDictionary<TKey, TValue>? Decode(ref Utf8JsonReader reader)
    {
        ...
        var values = ImmutableSortedDictionary.CreateBuilder<TKey, TValue>();
        ...
        while (reader.Read())
        {
            ...
            if (string.IsNullOrEmpty(keyStr))
                throw new JsonException("ImmutableSortedDictionary key cannot be null or empty.");

            var key = _stringToKeyMethod(keyStr);

            if (!reader.Read())
                throw new JsonException("Unexpected end of JSON while reading ImmutableSortedDictionary value.");
    ...
    /// <summary>
    /// Encodes an <see cref="ImmutableSortedDictionary{TKey, TValue}"/> into a JSON object.
    /// </summary>
    /// <param name="writer">The <see cref="Utf8JsonWriter"/> to write the JSON data to.</param>
    /// <param name="obj">The <see cref="ImmutableSortedDictionary{TKey, TValue}"/> to encode. Can be null.</param>
    public override void Encode(Utf8JsonWriter writer, ImmutableSortedDictionary<TKey, TValue>? obj)
    {
        if (obj is null)
        {
