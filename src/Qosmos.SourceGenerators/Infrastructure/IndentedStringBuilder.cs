...
namespace Qosmos.SourceGenerators.Infrastructure;
...
internal sealed class IndentedStringBuilder
{
    ...
    public IDisposable CreateScope()
    {
        ...
        return new IndentedStringBuilderScope(() =>
        {
            if (_indentation > 0)
                _indentation--;
    ...
    /// <summary>
    /// Creates a scope by appending a specified string, an opening brace, increasing the indentation,
    /// and returning a disposable action to close the scope.
    /// </summary>
    /// <param name="value">The string to append before the opening brace.</param>
    /// <returns>An <see cref="IDisposable"/> that closes the scope when disposed.</returns>
    public IDisposable CreateScope(string value)
    {
        _builder
            .Append(new string('\t', _indentation))
            .AppendLine(value)
    ...
    /// <summary>
    /// Creates a scope by appending a formatted string, an opening brace, increasing the indentation,
    /// and returning a disposable action to close the scope.
    /// </summary>
    /// <param name="value">The composite format string to append before the opening brace.</param>
    /// <param name="args">The arguments to format.</param>
    /// <returns>An <see cref="IDisposable"/> that closes the scope when disposed.</returns>
    public IDisposable CreateScope([StringSyntax(StringSyntaxAttribute.CompositeFormat)] string value, params object[] args)
    {
        _builder
            .Append(new string('\t', _indentation))
            .AppendFormat(value, args)
    ...
    /// </summary>
    private sealed record IndentedStringBuilderScope(Action Action) : IDisposable
    {
        /// <summary>
